...
import { useKecamatanStore, CHSHData } from '@/store/kecamatanStore';
...
export const StepCHSHKec: React.FC<StepCHSHProps> = ({ period, title, description }) => {
  const { chshMonthly, chshDasarian, chshPredictionPlus1, chshPredictionPlus2, chshPredictionPlus3, updateCHSHData } = useKecamatanStore();
  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      ...
      // Step 6: Calculate per kecamatan menggunakan point-in-polygon
      ...
      for (const feature of geojson.features) {
        ...
        // Create unique key using Kabupaten_Kecamatan format (handles duplicates like Jetis)
        const kecKey = `${kabName}_${kecName}`;
      ...
      toast({
        ...
        description: `CH/SH berhasil dihitung untuk ${Object.keys(results).length} kecamatan`,
      });
    } catch (error) {
