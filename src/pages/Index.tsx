...
const Index: React.FC = () => {
  ...
  const [nameField, setNameField] = useState("KAB_KOTA");
  ...
  useEffect(() => {
    fetch("/kab_kota.geojson")
      ...
      .then((data: GeoJSONCollection) => {
        ...
        // Set default name field if KAB_KOTA exists
        if (fields.includes("KAB_KOTA")) {
          setNameField("KAB_KOTA");
    ...
    setTimeout(() => {
      try {
        ...
        toast({
          ...
          description: `Menganalisis ${points.length.toLocaleString()} titik di ${computedResults.length} kabupaten/kota`,
        });
      } catch (err) {
  ...
  const handleDownload = useCallback(() => {
    ...
    link.href = url;
    link.download = `indikator_kekeringan_${period}.csv`;
    ...
    toast({
      ...
      description: `Hasil disimpan sebagai indikator_kekeringan_${period}.csv`,
    });
  }, [results, period]);
      ...
      <main className="container max-w-7xl mx-auto px-4 py-8">
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label>Peta Wilayah D.I. Yogyakarta</Label>
                  {results.length > 0 && (
                    <Select
          ...
          <section className="text-center text-sm text-muted-foreground py-4">
            <p>
              Unggah file CSV dengan kolom LAT, LONG (atau LON), CH (Curah Hujan dalam mm), dan SH (Sifat Hujan dalam %).
              Aplikasi akan menghitung indikator kekeringan untuk setiap kabupaten/kota di DIY.
            </p>
          </section>
        </div>
      </main>
